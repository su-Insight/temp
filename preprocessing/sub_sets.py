...
def random_sets(res):
    ...
    data = {}
    for who in flakys:
        data[who] = shuffle_df(pd.concat([flakys[who], safes[who]])).reset_index()
...
def tenfolds_half_sets(res):
    ...
    for i, _ in enumerate(flakys):
        for j in range(2):
            # 第一个合并：flakys[i][0] 和 safes[i][0]
            merged1 = pd.concat([flakys[i][0], safes[i][0]], ignore_index=True)
            shuffled1 = shuffle_df(merged1).reset_index(drop=True)  # 建议加 drop=True 避免索引重复

            # 第二个合并：flakys[i][1] 和 safes[i][1]
            merged2 = pd.concat([flakys[i][1], safes[i][1]], ignore_index=True)
            shuffled2 = shuffle_df(merged2).reset_index(drop=True)
